...
class LLMService:
    ...
    def remove_duplicates(self, html_content: str) -> str:
        ...
        뉴스 제공사가 다양하기 때문에 일부 뉴스는 중복되어 수집되어 있어서 중복은 없도록 제거해야해.
        현재의 html 파일 구조를 유지하고 중복된 뉴스들은 한개만 남기고 삭제해서 html 파일 생성해.
        즉, html의 style은 변경하지 말고, 뉴스제목(클릭시 링크로 이동)과 요약으로 구성되고, 뉴스간에는 구분이 있어야해.
        html을 바로 사용할 예정이므로 삭제한 중복 뉴스목록이나 자동으로 생성되었다는 등의 진행 과정설명과 같은 불필요한 부분은 표시하지마."""
